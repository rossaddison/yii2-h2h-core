...
?>
<div>
<img src="<?= Utilities::getPlaceholderRelativeUrl().InvoiceHelper::invoice_logo();?>" height="60" width="60">
<br>
<br>
<?php
    ...
    //build company footer from settings
    $pdf_invoice_footer = '';
...
<h4>
    <?php
        if ((!empty($model->reference)) && ($model->reference <> '??')){
            $invoiceref = $model->reference;
             echo '<h4 style="background-color:lightblue">Your ref: '.$invoiceref.'</h4>';
        }    
    ?>    
</h4>
<br>
...
<?php
  ...
  //print out the salesorderdetails aka individual window cleans for each daily clean
...
<watermarktext content="<?php Utilities::trans('paid'); ?>" alpha="0.3"></watermarktext>
